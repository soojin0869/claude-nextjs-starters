...
- **Task 016: 어드민 대시보드 페이지 구현**
  - 어드민 메인 페이지(`/admin`)를 단순 견적서 목록에서 통합 대시보드로 개편
  - 사이트 타이틀 "견적서 관리 시스템"으로 변경 (`components/admin/admin-header.tsx`)
  - 견적서 관리 섹션: 상태별 통계 카드(전체 / 확정됨 / 발송됨 / 초안) UI 및 데이터 집계 로직 구현
  - 클라이언트 관리 섹션: 인보이스 데이터에서 추출한 고객사 목록 및 요약 정보(고객사명, 견적서 수, 총 금액 등) 표시
  - 최근 활동 섹션: 최근 5건의 견적서 목록을 카드 형태로 표시
  - 기존 견적서 목록은 대시보드 내 "모든 견적서 보기" 링크/버튼을 통해 별도 경로(예: `/admin/invoices`)로 접근하도록 분리
  - 새 컴포넌트 작성:
    - `components/admin/dashboard/stat-card.tsx` - 상태별 통계 카드 컴포넌트
    - `components/admin/dashboard/recent-activity-list.tsx` - 최근 활동 목록 컴포넌트
    - `components/admin/dashboard/client-summary.tsx` - 클라이언트 요약 컴포넌트
  - 기존 견적서 목록 라우트 분리 및 대시보드와 목록 페이지 간 네비게이션 연결
  - 반응형 그리드 레이아웃 적용 (모바일/태블릿/데스크톱) 및 다크모드 색상 변수 적용
  - 작업 완료 기준: `npm run check-all` 통과 및 `npm run build` 성공
  - **테스트 체크리스트 (Playwright MCP)**:
    - 인증된 상태에서 `/admin` 접근 시 통합 대시보드가 정상 렌더링되는지 확인
    - 헤더 사이트 타이틀이 "견적서 관리 시스템"으로 표시되는지 확인
    - 견적서 관리 섹션의 상태별 통계 카드 수치가 실제 데이터와 일치하는지 확인
    - 클라이언트 관리 섹션에 고객사 목록 및 요약 정보가 표시되는지 확인
    - 최근 활동 섹션에 최근 5건의 견적서가 노출되는지 확인
    - "모든 견적서 보기" 링크 클릭 시 견적서 전체 목록 페이지로 이동하는지 확인
    - 미인증 상태에서 `/admin` 접근 시 `/admin/login`으로 리다이렉트되는지 확인
    - 다크모드 토글 시 대시보드 컴포넌트들의 색상이 정상 전환되는지 확인
    - 모바일 뷰포트에서 반응형 레이아웃이 정상 동작하는지 확인
